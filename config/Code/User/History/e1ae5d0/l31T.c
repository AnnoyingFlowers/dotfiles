...
void Database_write(struct Connection *conn) {
    ...
    for (int i = 0; i < conn->db->max_rows; i++) {
        ...
        rc =
            fwrite(address->name, sizeof(char), conn->db->max_data, conn->file);
        if (rc != conn->db->max_data) die("Failed to write address->name");

        rc = fwrite(address->email, sizeof(char), conn->db->max_data,
                    conn->file);
...
int main(int argc, char *argv[]) {
    if (argc < 5)
        die("USAGE: ex17 <dbfile> <max_rows> <max_data> <action> [action "
            "params]");
