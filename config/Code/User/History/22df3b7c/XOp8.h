...
static inline bool difftest_check_reg(const char *name, vaddr_t pc, word_t ref,
    ...
    if (ref != dut) {
        // 为了把错误信息整成一行显示完
        printf("%s $%s is different at pc = " FMT_WORD ", right = " FMT_WORD
               ", wrong = " FMT_WORD ", diff = " FMT_WORD "\n",
               ANSI_FMT("DIFFTEST FAILED", ANSI_FG_RED), name, pc, ref, dut,
               ref ^ dut);
        log_write("%s $%s is different at pc = " FMT_WORD ", right = " FMT_WORD
                  ", wrong = " FMT_WORD ", diff = " FMT_WORD "\n",
                  ANSI_FMT("DIFFTEST FAILED", ANSI_FG_RED), name, pc, ref, dut,
                  ref ^ dut);
        return false;
