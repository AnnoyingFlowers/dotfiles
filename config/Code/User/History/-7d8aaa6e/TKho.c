...
char *test_read_write() {
    ...
    mu_assert(bytes_written3 == 6, "Failed to write data to buffer");
    mu_assert(RingBuffer_full(buffer),
              "RingBuffer should still be full after partial writing");
    mu_assert(
        RingBuffer_available_data(buffer) == DEFAULT_LEN,
        "Available data should still be DEFAULT_LEN after partial writing");
    mu_assert(RingBuffer_available_space(buffer) == 0,
