...
char *test_read_write() {
    // 测试初始状态
    assert(RingBuffer_empty(buffer) && "RingBuffer should be empty initially");
    assert(!RingBuffer_full(buffer) &&
           "RingBuffer should not be full initially");
    assert(RingBuffer_available_data(buffer) == 0 &&
           "Available data should be 0 initially");
    assert(RingBuffer_available_space(buffer) == 5 &&
           "Available space should be 5 initially");
    ...
    assert(bytes_read == 3 && "Failed to read data from buffer");
    assert(memcmp(data + 5, target, bytes_read) == 0 &&
           "Data read doesn't match data written");

    // 再次写入数据，不会覆盖之前的数据
    char data3[] = "ABCDE";
    int bytes_written3 = RingBuffer_write(buffer, data3, sizeof(data3) - 1);
    assert(bytes_written3 == 2 && "Failed to write data to buffer");
    assert(RingBuffer_full(buffer) &&
    ...
    assert(bytes_read == 5 && "Failed to read data from buffer");
    assert(memcmp(data + 5, target + 3, bytes_read) == 0 &&
           "Data read doesn't match data written");
