...
char *test_read_write() {
    ...
    mu_assert(RingBuffer_empty(buffer), "RingBuffer should be empty");
    mu_assert(!RingBuffer_full(buffer), "RingBuffer should not be full");

    // 情况 2: 缓冲区已满
    buffer->start = 0;
    buffer->end = 5;

    mu_assert(!RingBuffer_empty(buffer), "RingBuffer should not be empty");
    mu_assert(RingBuffer_full(buffer), "RingBuffer should be full");
