...
char *test_read_write() {
    ...
    mu_assert(bytes_written == sizeof(data), "Failed to write data to buffer");
    mu_assert(!RingBuffer_full(buffer),
              "RingBuffer should not be full after writing");
    mu_assert(RingBuffer_available_data(buffer) == sizeof(data),
              "Available data should be sizeof(data) after writing");
    mu_assert(
        RingBuffer_available_space(buffer) == DEFAULT_LEN - sizeof(data),
        "Available space should be DEFAULT_LEN - sizeof(data) after writing");

    // 再次写入数据
    char data2[] = "12345";
    int bytes_written2 = RingBuffer_write(buffer, data2, sizeof(data2));
    mu_assert(bytes_written2 == sizeof(data2),
              "Failed to write data2 to buffer");
    mu_assert(RingBuffer_full(buffer),
              "RingBuffer should still be not full after overwriting");
    mu_assert(RingBuffer_available_data(buffer) == DEFAULT_LEN,
              "Available data should be DEFAULT_LEN after overwriting");
    ...
    mu_assert(memcmp(data, target, bytes_read) == 0,
    ...
    mu_assert(bytes_written3 == sizeof(data3),
              "Failed to write data to buffer");
    mu_assert(!RingBuffer_full(buffer),
              "RingBuffer should not be full after partial writing");
    mu_assert(
        RingBuffer_available_data(buffer) == sizeof(data3),
        "Available data should still be sizeof(data3) after partial writing");
    mu_assert(RingBuffer_available_space(buffer) == DEFAULT_LEN - sizeof(data3),
              "Available space should still be DEFAULT_LEN - sizeof(data3) "
              "after partial writing");

    // 读取所有数据
    bytes_read =
        RingBuffer_read(buffer, target, RingBuffer_available_data(buffer));
    mu_assert(bytes_read == sizeof(data3), "Failed to read data from buffer");
    mu_assert(memcmp(data3, target, bytes_read) == 0,
              "Data read doesn't match data written");
    ...
    mu_assert(RingBuffer_available_space(buffer) == DEFAULT_LEN,
              "Available space should be DEFAULT_LEN after reading all data");
