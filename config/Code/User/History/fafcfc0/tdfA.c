...
// the size of a struct is approximately equal to their sum byte size
// but when its members are not byte-aligned by 4 or 8,
// C will add some padding bytes.
struct Address {
    int id;
    int set;
    char *name;
...
void Address_print(struct Address *addr) {
    printf("%d %s %s %s\n", addr->id, addr->name, addr->phone, addr->email);
}
...
void Database_load(struct Connection *conn) {
    ...
    for (int i = 0; i < conn->db->MAX_ROWS; i++) {
        ...
        row->phone = malloc(conn->db->MAX_DATA);
        row->email = malloc(conn->db->MAX_DATA);

        rc = fread(row->name, 1, conn->db->MAX_DATA, conn->file);
        if (rc != conn->db->MAX_DATA) die(conn, "Failed to load name.");
        rc = fread(row->phone, 1, conn->db->MAX_DATA, conn->file);
        if (rc != conn->db->MAX_DATA) die(conn, "Failed to load phone.");
        rc = fread(row->email, 1, conn->db->MAX_DATA, conn->file);
...
void Database_write(struct Connection *conn) {
    ...
    for (int i = 0; i < conn->db->MAX_ROWS; i++) {
        ...
        rc = fwrite(row->phone, 1, conn->db->MAX_DATA, conn->file);
        if (rc != conn->db->MAX_DATA) die(conn, "Failed to write phone.");
...
void Database_create(struct Connection *conn, char *str_MAX_DATA,
    ...
    for (int i = 0; i < conn->db->MAX_ROWS; i++) {
        ...
        addr->phone = malloc(conn->db->MAX_DATA);
...
void Database_get(struct Connection *conn, int id) {
    if (id >= conn->db->MAX_ROWS) die(conn, "There's not that many records.");
    if (id == -1) {
        die(conn, "Unable to find any records.");
        return;
    }
...
int Database_find(struct Connection *conn, char *info) {
    for (int i = 0; i < conn->db->MAX_ROWS; i++) {
        struct Address *row = conn->db->rows[i];
        if (row->set == 1 &&
            (strcmp(row->name, info) == 0 || strcmp(row->email, info) == 0 ||
             strcmp(row->phone, info) == 0)) {
            return i;
        }
    }
    return -1;
...
int main(int argc, char *argv[]) {
    ...
    switch (action) {
        case 'c':
            if (argc != 5) die(conn, "Need MAX_DATA, MAX_ROWS to set");
        ...
        case 's':
            if (argc != 7) die(conn, "Need id, name, phone, email to set");
        ...
        case 'f':
            if (argc != 4) die(conn, "Need name or phone or email to find");

            int id = Database_find(conn, argv[3]);
            Database_get(conn, id);
            break;
