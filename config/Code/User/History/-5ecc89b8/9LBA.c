...
void init_elf(const char* elf_file) {
    ...
    for (int i = 0; i < symtab->sh_size / symtab->sh_entsize; ++i) {
        if (ELF32_ST_TYPE(symbols[i].st_info) == STT_FUNC) {
            ...
            funtab.funcs = new_funcs;

            // 设置新的Func
            funtab.funcs[funtab.func_cnt].func_name =
                strdup(&strtab[symtab[i].st_name]);
            funtab.funcs[funtab.func_cnt].start =
                symtab[i].st_value;  // 假设 start 是地址值
            funtab.funcs[funtab.func_cnt].end =
                symtab[i].st_value +
                symtab[i].st_size;  // 假设 end 是地址值加上大小
