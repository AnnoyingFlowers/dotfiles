...
void iringbuf_display() {
    int dis_n = BUFFER_ROWS > iringbuf.cnt ? iringbuf.cnt : BUFFER_ROWS;
...
void init_elf(const char* elf_file) {
    // 未加载ELF文件，使用内置image
    if (strcmp(elf_file, ".elf") == 0) {
        Log("No ELF file is given. Use the default build-in image.");
        return;
    }
    // printf("%s\n", elf_file);
    ...
    size_t nmemb = fread(&ehdr, sizeof(ehdr), 1, fp);
    Assert(nmemb == 1, "Can't read required bytes\n");
    ...
    nmemb = fread(shdr, ehdr.e_shentsize, ehdr.e_shnum, fp);
    Assert(nmemb == ehdr.e_shnum, "Can't read required bytes\n");
    ...
    nmemb = fread(symbols, symtab->sh_size, 1, fp);
    Assert(nmemb == 1, "Can't read required bytes\n");

    // 读取字符串表
    char* strs = malloc(strtab->sh_size);
    fseek(fp, strtab->sh_offset, SEEK_SET);
    nmemb = fread(strs, strtab->sh_size, 1, fp);
    Assert(nmemb == 1, "Can't read required bytes\n");
