...
void ftrace_call(vaddr_t pc, vaddr_t dnpc) {
    for (int i = 0; i < funtab.func_cnt; i++) {
        if (dnpc >= funtab.funcs[i].start && dnpc < funtab.funcs[i].end) {
            char* spaces = (char*)malloc(g_nr_call_func * 2 + 1);
            memset(spaces, ' ', g_nr_call_func * 2);
            flog_write(FMT_PADDR ": %scall [%s@" FMT_PADDR "]", pc, spaces,
...
void ftrace_ret(vaddr_t pc) {
    for (int i = 0; i < funtab.func_cnt; i++) {
        if (pc >= funtab.funcs[i].start && pc < funtab.funcs[i].end) {
            char* spaces = (char*)malloc(g_nr_call_func * 2);
            memset(spaces, ' ', g_nr_call_func * 2);
            flog_write(FMT_PADDR ": %sret [%s]", pc, spaces,
                       funtab.funcs[i].func_name);
            g_nr_call_func--;
            free(spaces);
            break;
