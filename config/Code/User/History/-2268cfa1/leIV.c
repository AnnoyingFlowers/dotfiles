...
void List_bottom_up_merge(List *A, ListNode *iLeft_node, ListNode *iRight_node,
                          ListNode *A_iEnd_node, ListNode *B_iEnd_node, List *B,
                          List_compare cmp) {
    ListNode *i = iLeft_node, *j = iRight_node;
    int is_pass_iEnd = false;
    for (ListNode *k = iLeft_node; k != B_iEnd_node; k = k->next) {
        if (i != iRight_node && (j >= iEnd || cmp(i->value, j->value) <= 0)) {
            k->value = i->value;
            i = i->next;
        } else {
            k->value = j->value;
...
void List_bottom_up_merge_sort(List *A, List *B, int n, List_compare cmp) {
    for (int width = 1; width < n; width <<= 1) {
        for (int i = 0; i < n; i += width << 1) {
            ...
            ListNode *B_iEnd_node = B->first;
            for (int tp = 0; tp < iEnd; tp++) {
                B_iEnd_node = B_iEnd_node->next;
            }

            List_bottom_up_merge(A, iLeft_node, iRight_node, A_iEnd_node,
                                 B_iEnd_node, B, cmp);
        }
        A = List_copy(B);
    }
