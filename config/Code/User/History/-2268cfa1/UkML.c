...
inline void List_bottom_up_merge(List *A, ListNode *A_iLeft_node,
                                 ...
                                 List_compare cmp) {
    ListNode *i = A_iLeft_node, *j = A_iRight_node;
    int is_pass_iEnd = false;
    for (ListNode *k = B_iLeft_node; k != B_iEnd_node; k = k->next) {
        if (i != A_iRight_node &&
...
void List_bottom_up_merge_sort(List *A, List *B, int n, List_compare cmp) {
    for (int width = 1; width < n; width <<= 1) {
        for (int i = 0; i < n; i += width << 1) {
            ...
            ListNode *B_iRight_node = A->first;
            for (int tp = 0; tp < iRight; tp++) {
                B_iRight_node = B_iRight_node->next;
            }
