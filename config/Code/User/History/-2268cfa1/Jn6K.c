...
inline void List_bottom_up_merge(ListNode *A_iLeft_node, ListNode *B_iLeft_node,
...
List *List_bottom_up_merge_sort(List *A, List *B, int n, List_compare cmp) {
    for (int width = 1; width < n; width <<= 1) {
        for (int i = 0; i < n; i += width << 1) {
            int iLeft = i, iRight = min(i + width, n),
            ...
            List_bottom_up_merge(A_iLeft_node, B_iLeft_node, A_iRight_node,
                                 A_iEnd_node, cmp);
        }
        A = List_copy(B);
    }
