...
inline void List_bottom_up_merge(List *A, ListNode *iLeft_node,
                                 ListNode *iRight_node, ListNode *A_iEnd_node,
                                 ListNode *B_iEnd_node, List *B,
                                 List_compare cmp) {
    ListNode *i = iLeft_node, *j = iRight_node;
    int is_pass_iEnd = false;
    for (ListNode *k = iLeft_node; k != B_iEnd_node; k = k->next) {
        if (i != iRight_node &&
            (is_pass_iEnd || cmp(i->value, j->value) <= 0)) {
            k->value = i->value;
            i = i->next;
        } else {
            k->value = j->value;
