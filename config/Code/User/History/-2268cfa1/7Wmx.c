...
whole array is sorted. for (width = 1; width < n; width = 2 * width)
    {
...
void List_bottom_up_merge(List *A, ListNode *iLeft_node, ListNode *iRight_node,
                          ListNode *iEnd_node, List *B, List_compare cmp) {
    ListNode *i = iLeft_node, *j = iRight_node;
    int is_pass_iEnd = false;
    for (ListNode *k = iLeft_node; k != iEnd_node; k = k->next) {
        if (i != iRight_node && (j >= iEnd || cmp(A[i], A[j]) <= 0)) {
            B[k] = A[i];
            i++;
        } else {
            B[k] = A[j];
...
void List_bottom_up_merge_sort(List *A, List *B, int n, List_compare cmp) {
    for (int width = 1; width < n; width <<= 1) {
        for (int i = 0; i < n; i += width << 1) {
            ...
            ListNode *iLeft_node = A->first;
            for (int tp = 0; tp < iLeft; tp++) {
                iLeft_node = iLeft_node->next;
            }

            ListNode *iRight_node = A->first;
            for (int tp = 0; tp < iRight; tp++) {
                iRight_node = iRight_node->next;
            }

            ListNode *iEnd_node = A->first;
            for (int tp = 0; tp < iEnd; tp++) {
                iEnd_node = iEnd_node->next;
            }

            List_bottom_up_merge(A, iLeft_node, iRight_node, iEnd_node, B, cmp);
        }
        A = List_copy(B);
    }
}
