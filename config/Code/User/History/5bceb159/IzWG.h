 ...
 *
 * @copyright GFDL
 * @copyright Creative Commons Attribution-ShareAlike 3.0 Unported License (CC
 * BY-SA 3.0)
 */
#ifndef RNG_BUF_H
#define RNG_BUF_H
...
rng_buf_t *rng_buf_create(const char *name, std::size_t min_size,
                          _Bool use_file_in_tmp);
void rng_buf_destroy(rng_buf_t **rb);

static inline unsigned char *rng_buf_write_address(struct rng_buf *rb) {
    return rb->base + rb->write_offset_bytes;
}

static inline void rng_buf_write_advance(struct rng_buf *rb, uint64_t size) {
    rb->write_offset_bytes += size;
}

static inline unsigned char *rng_buf_read_address(struct rng_buf *rb) {
    return rb->base + rb->read_offset_bytes;
}

static inline void rng_buf_read_advance(struct rng_buf *rb, uint64_t size) {
...
static inline uint64_t rng_buf_len(struct rng_buf *rb) {
    return rb->write_offset_bytes - rb->read_offset_bytes;
}

static inline uint64_t rng_buf_free_bytes(struct rng_buf *rb) {
